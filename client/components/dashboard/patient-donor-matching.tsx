...
const PatientDonorMatching = ({ patients, selectedPatient }: PatientDonorMatchingProps) => {
  ...
  const runMatching = async (id: string) => {
    ...
    try {
      // API call to the matching endpoint
      const response = await fetch('/api/donor-matching', {
        method: 'POST',
        headers: {
      ...
      setError("Failed to fetch matching donors. Please try again.");
      
      // Fallback to client-side matching if the API fails
      const fallbackMatches = performLocalMatching(patient);
  ...
  const performLocalMatching = (patient: Patient): DonorWithScore[] => {
    return donorMockData
      .filter(donor => {
  ...
  return (    <div className="space-y-6">
    ...
    {hasRunSearch && !isLoading && matchResults.length > 0 && (
      <Card>
